...
# Applica il filtro di Savitzky-Golay a tutto il dataset per tutte le colonne numeriche
window_length = 9  # Deve essere un numero dispari
poly_order = 2
...
for column in columns_to_filter:
    if df_filtered[column].dtype == np.float64:
        df_filtered[column] = savgol_filter(df_filtered[column], window_length=window_length, polyorder=poly_order)
...
df_filtered.to_csv('./data/Dataset/Data_PV_filtro_savgol.csv', index=False)
